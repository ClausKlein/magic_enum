...
all: ## Make all with cmake workflow preset
	cmake --version
	cmake -S . -B $(_build_path) -G $(CMAKE_GENERATOR) -D MAGIC_ENUM_OPT_BUILD_EXAMPLES=YES -D CMAKE_CXX_STANDARD=26 \
		-D MAGIC_ENUM_MODULE_IMPORT_STD=YES -D MAGIC_ENUM_USE_MODULES=YES -D CMAKE_BUILD_TYPE=$(CMAKE_BUILD_TYPE) \
		-D CMAKE_EXPERIMENTAL_CXX_IMPORT_STD="f35a9ac6-8463-4d38-8eec-5d6008153e7d" # for cmake 4.4.x only
