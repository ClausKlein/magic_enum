...
function(
  ...
  # $<TARGET_PROPERTY:${TARGET},NAME> INTERFACE_LINK_DIRECTORIES INTERFACE_LINK_LIBRARIES
  ...
  # Only use prefix if paths are not absolute like they are with nix See also:
  ...
  if(IS_ABSOLUTE "${_INSTALL_LIB_DIR}")
    list(APPEND header
         "$<IF:$<OR:$<BOOL:${PUBLIC_LIBS}>,${NEEDS_LIB_DIR}>,libdir=${_INSTALL_LIB_DIR},>"
    )
  else()
    list(
      APPEND header
      "$<IF:$<OR:$<BOOL:${PUBLIC_LIBS}>,${NEEDS_LIB_DIR}>,libdir=\${prefix}/${_INSTALL_LIB_DIR},>"
    )
  endif()
  ...
  if(IS_ABSOLUTE "${_INSTALL_INCLUDE_DIR}")
    ...
    list(APPEND header
         "$<IF:$<BOOL:${PUBLIC_INCLUDES}>,includedir=\${prefix}/${_INSTALL_INCLUDE_DIR},>"
    )
  endif()
  ...
  set(OTHER_INCLUDE_FLAGS "-I$<JOIN:$<REMOVE_DUPLICATES:${PUBLIC_INCLUDES}>, -I>"
  )# Not needed, we can only get build interface flags here. Insert them after -I\${includedir} if
   # you find a way to fix/workaround it

  # Here is a workaround to inability to use TARGET_LINKER_FILE_NAME for targets not involving
  # library generation. Strangely $<IF evaluates both branches, not only the one taken, which causes
  # an error We workaround it by generating the subexpression source using $<IF and then evaluating
  # it using $<GENEX_EVAL Of course we could have used conditional expressions on CMake script part,
  # but I have decided to implement it in generator expressions part, so hypthetically all the
  # expressions can be merged into a single file and this function can be made simple

  set(ESCAPED_GENEXPR_BEGINNING "$<1:$><") # A hack because there is no escape for `$` or `<` or
                                           # `$<`.  So we just disrupt $< into pieces
  set(CURRENT_LIB_ESCAPED_BINARY_NAME
      "${ESCAPED_GENEXPR_BEGINNING}TARGET_LINKER_FILE_NAME:${TARGET}$<ANGLE-R>"
  )
  set(LINK_CURRENT_LIB_FLAG
      "$<GENEX_EVAL:$<IF:${NEEDS_LIBS},-l:${CURRENT_LIB_ESCAPED_BINARY_NAME},>>"
  )
  ...
  if(CONFIGURE_TIME_IS_OBJECT)
    ...
    if(IS_TARGET_OBJECTS_CONFIGURE_TIME_UNAVAILABLE)
      message(
        WARNING
          "CMake is shit: There is (at least was at the time of writing of this code) no generator expression to get only basenames of object files. They are also unavailable at configure stage. And there were no CMake generator expressions for making replacements in strings. So we workaround this."
      )
      ...
      add_custom_command(
        OUTPUT "${TARGET_OBJECTS_FILE}" COMMENT "A dummy command to workaround cmake limitations"
      )
      add_custom_target("${OBJECTS_FILE_RETRIEVAL_TARGET_NAME}" DEPENDS "${TARGET_OBJECTS_FILE}")

      add_custom_command(
        OUTPUT "${PKG_CONFIG_FILE_NAME_FINISHED}" PRE_BUILD
        COMMAND
          ${CMAKE_COMMAND} "-DobjectsFile=\"${TARGET_OBJECTS_FILE}\""
          "-DpkgConfigFileUnlinished=\"${PKG_CONFIG_FILE_NAME_UNFINISHED}\""
          "-DpkgConfigFileFinal=\"${PKG_CONFIG_FILE_NAME_FINISHED}\"" "-P"
          "${GEN_PKG_CONFIG_WORKAROUNDS_BUILD_TIME_SCRIPTS}/getObjectFilesBaseNames.cmake"
        MAIN_DEPENDENCY "${TARGET_OBJECTS_FILE}"
        DEPENDS "${PKG_CONFIG_FILE_NAME_UNFINISHED}"
        COMMENT
          "Working around CMake limitations about getting list of basenames of object files and about lack of generator expressions to modify strings: ${PKG_CONFIG_FILE_NAME_UNFINISHED} + ${TARGET_OBJECTS_FILE} -> ${PKG_CONFIG_FILE_NAME_FINISHED}"
      )
      add_custom_target(
        "${PKGCONFIG_PATCH_TARGET_NAME}" ALL DEPENDS "${PKG_CONFIG_FILE_NAME_FINISHED}"
      )
      add_dependencies(
      ...
      message(
        FATAL_ERROR
          "This branch is unimplemented because CMake lacked the needed functionality at the time"
      )
  ...
  list(
    APPEND
    libSpecific
    "$<IF:$<OR:$<BOOL:${PUBLIC_LIBS}>,${NEEDS_LIBS},${IS_OBJECT}>,Libs: -L\${libdir} ${LINK_CURRENT_LIB_FLAG} ${LINK_CURRENT_OBJECT_FLAG} $<IF:$<BOOL:${PUBLIC_LIBS}>,-l$<JOIN:$<REMOVE_DUPLICATES:${PUBLIC_LIBS}>, -l>,>,>\n$<IF:$<OR:$<BOOL:${PUBLIC_INCLUDES}>,$<BOOL:${PUBLIC_COMPILE_FLAGS}>>,Cflags: -I\${includedir}$<IF:$<BOOL:${PUBLIC_COMPILE_FLAGS}>, $<JOIN:$<REMOVE_DUPLICATES:${PUBLIC_COMPILE_FLAGS}>,>,>,>"
  )
  ...
  if(CONFIGURE_TIME_IS_OBJECT)
    ...
    add_custom_target(
      "${PKGCONFIG_DUMMY_UNFINISHED_GEN_TARGET_NAME}" DEPENDS "${PKG_CONFIG_FILE_NAME_UNFINISHED}"
    )

    install(
      FILES "${PKG_CONFIG_FILE_NAME_FINISHED}"
    ...
    file(
      GENERATE
      OUTPUT "${PKG_CONFIG_FILE_NAME}"
      CONTENT "${libSpecific}"
    )

    install(
      FILES "${PKG_CONFIG_FILE_NAME}"
      DESTINATION "${_INSTALL_LIB_DIR}/pkgconfig"
      COMPONENT "${_COMPONENT}"
    )
  endif()
